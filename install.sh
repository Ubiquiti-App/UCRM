...
GIT_REPOSITORY="U-CRM/billing/master"
...
download_docker_compose_files() {
    if [ ! -f "/home/${UCRM_USER}/docker-compose.yml" ]; then
        ...
        if (echo "${INSTALL_VERSION}" | grep -q "beta"); then
            curl -o "/home/${UCRM_USER}/docker-compose.yml" "https://raw.githubusercontent.com/${GIT_REPOSITORY}/docker-compose.beta.yml"
        else
            curl -o "/home/${UCRM_USER}/docker-compose.yml" "https://raw.githubusercontent.com/${GIT_REPOSITORY}/docker-compose.yml"
        fi

        curl -o "/home/${UCRM_USER}/docker-compose.migrate.yml" "https://raw.githubusercontent.com/${GIT_REPOSITORY}/docker-compose.migrate.yml"
        curl -o "/home/${UCRM_USER}/docker-compose.env" "https://raw.githubusercontent.com/${GIT_REPOSITORY}/docker-compose.env"

        sed -i -e "s/    image: ubnt\/ucrm-billing:.*/    image: ubnt\/ucrm-billing:${INSTALL_VERSION}/g" "/home/${UCRM_USER}/docker-compose.yml"
        sed -i -e "s/    image: ubnt\/ucrm-billing:.*/    image: ubnt\/ucrm-billing:${INSTALL_VERSION}/g" "/home/${UCRM_USER}/docker-compose.migrate.yml"
    ...
    while true; do
        ...
        case "${PORT}" in
            [yY]*)
                ...
                echo "#used only in installation" >> "/home/${UCRM_USER}/docker-compose.env"
